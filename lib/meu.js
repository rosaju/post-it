...
function atualizarSecao(secao) {
    ...
    for (var posicao = 0; posicao < notas.length; posicao++) {
        if (notas[posicao].editando) {
            ...
            conteudoSecao += '<form class="note">' + '<input class="note__title" type="text" name="title" value="' + notas[posicao].titulo + '"placeholder="Título" autofocus />' + '<textarea class="note__body" name="body" rows="5" placeholder="Criar uma nota...">' + notas[posicao].texto + '</textarea>' + '<button class="note__control" type="button" onclick="adicionarNota(this.parentElement.title, this.parentElement.body, this.parentElement, this.parentElement.parentElement, ' + posicao + ')">' + 'Concluído' + '</button>' + '</form>';
        } else {
            conteudoSecao += '<form class="note" onclick="editaFormulario(' + posicao + ', this.parentElement)">' + '<button class="note__control" type="button" onclick="removerNota(evento,' + posicao + ', this.form.parentElement)">' + '<i class="fa fa-times" aria-hidden="true"></i>' + '</button>' + '<h1 class="note__title">' + notas[posicao].titulo + '</h1>' + '<p class="note__body">' + notas[posicao].texto + '</p>' + '</form>';
        }
    }
